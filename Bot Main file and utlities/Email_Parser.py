...
def _evaluate_candidate(
    ...
    *,
    ...
    validation_score += 1
    if normalized_expected_payment_note:
        if not _message_contains_expected_payment_note(
            parsed_message.headers,
            parsed_message.subject,
            parsed_message.plain_text,
            parsed_message.html_text,
...
def _select_best_rejection(evaluations: list[CandidateEvaluation]) -> PaymentParserResult:
    if not evaluations:
        return _build_default_result("no candidate messages found")
...
def _list_recent_inbox_message_ids(gmail_service: Any) -> list[str]:
    response = (
        gmail_service.users()
        .messages()
        .list(
...
def _fetch_recent_candidates(gmail_service: Any) -> list[ParsedGmailMessage]:
    parsed_messages: list[ParsedGmailMessage] = []
    for message_id in _list_recent_inbox_message_ids(gmail_service):
        gmail_message = (
            gmail_service.users()
            .messages()
            .get(
...
def check_payment_email(
    *,
    ...
    try:
        ...
        parsed_messages = _fetch_recent_candidates(gmail_service)
    ...
    if selected_result is None:
        selected_result = _select_best_rejection(evaluations)
